...
local function Update_Settings( ply )
	...
	if ply:GetInfoNum( pb_.. "enabled", 0 ) ~= 0 then
		...
		status.ply = ply
		status.sleep = ply:GetInfoNum( pb_.. "sleep", 0 ) ~= 0
		status.nocollide = ply:GetInfoNum( pb_.. "nocollide", 0 ) ~= 0
		status.limit_rotation = ply:GetInfoNum( pb_.. "norotate", 0 ) ~= 0
		status.snapmove = ply:GetInfoNum( pb_.. "snapmove", 0 ) ~= 0
		
		if ply:GetInfoNum( pb_.. "snap_position", 0 ) ~= 0 then
			...
			if snap_position ~= Vector(0,0,0) then
				...
				status.snap_boxcentre = ply:GetInfoNum( pb_.. "snap_boxcentre", 0 ) ~= 0
			end
		end
		
		if ply:GetInfoNum( pb_.. "snap_angles", 0 ) ~= 0 then
			local snap_angles = Angle(0,0,0)
			
			snap_angles.p = math.max( 0, ply:GetInfoNum( pb_.. "angles_p", 0 ) )
			snap_angles.y = math.max( 0, ply:GetInfoNum( pb_.. "angles_y", 0 ) )
			snap_angles.r = math.max( 0, ply:GetInfoNum( pb_.. "angles_r", 0 ) )
...
local function limit_rotation( ent )
	...
	if !status.ballsockets then
		...
		status.ballsockets[2] = constraint.AdvBallsocket( ent, game.GetWorld(), 0, 0, pos, pos, 0, 0, -180, 0, -180, 180, 0, 180, 0, 0, 0, 1, 0 )
		status.ballsockets[3] = constraint.AdvBallsocket( ent, game.GetWorld(), 0, 0, pos, pos, 0, 0, -180, -180, 0, 180, 180, 0, 0, 0, 0, 1, 0 )
	end
end
...
local function snap_angles( ent, status )
	if status.snap_angles then
		...
		if snap then
			local pIsZero = (snap.p == 0)
			local yIsZero = (snap.y == 0)
			local rIsZero = (snap.r == 0)

			local offset = Angle(
				(pIsZero and 0) or ( ang.p + snap.p / 2 ) % snap.p - snap.p / 2,
				(yIsZero and 0) or ( ang.y + snap.y / 2 ) % snap.y - snap.y / 2,
				(rIsZero and 0) or ( ang.r + snap.r / 2 ) % snap.r - snap.r / 2
			)
...
Phys_Grab = function( ply, ent )
	if not IsValid( ent ) or not IsValid( ply ) or IsValid( ent:GetParent() ) or ent:GetClass() == "player" then return end
	if CPPI and not ent:CPPICanPhysgun( ply ) then return end
